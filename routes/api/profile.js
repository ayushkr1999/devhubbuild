...
// @desc    ctreate or update users progfiile
// @access  Public
...
// @desc    get profule by user id
// @access  Public
router.get('/user/:user_id',async(req,res)=>{
    try {
        const profile = await Profile.findOne({user:req.params.user_id}).populate('user',['name','avatar'])
        if(!profile){
            return res.status(400).json({msg:"no such profile for this user"})
        }
        res.json(profile) 
    } catch (err) { 
...
router.delete('/experience/:exp_id',auth,async(req,res)=>{
    try{
        ...
        //get remocveindex
        const removeindex =profile.experience.map(itme=>itme.id).indexOf(req.params.exp_id)
...
router.delete('/education/:edu_id',auth,async(req,res)=>{
    try{
        ...
        //get remocveindex
        const removeindex =profile.education.map(itme=>itme.id).indexOf(req.params.edu_id)
